...
set -o pipefail
java -Xmx${java_heap_memory_initial} -jar ${path_herramientas}/bptools.jar ${parametros} ${input_vcf} ${sample_name}.${nombre_step}.vcf
}


output {
    
    File bptools_out = "${sample_name}.${nombre_step}.vcf"
    }
...
set -o pipefail
java -Xmx${java_heap_memory_initial} -jar ${path_herramientas}/snpEff.jar ${reference_version} -hgvs \
-t -lof -noStats -canon -onlyProtein -c ${path_herramientas}/snpEff.config \
${input_vcf} > ${sample_name}.step1_SnpEff.vcf 
}

output {
    File step1_snpeff = "${sample_name}.step1_SnpEff.vcf"
...
command {
set -o pipefail
java -Xmx${java_heap_memory_initial} -jar ${path_herramientas}/SnpSift.jar ${parametros} ${database} \
${input_vcf} > ${sample_name}.${nombre_step}.vcf

}

output {
    File salida_Snpsift = "${sample_name}.${nombre_step}.vcf"
}
...
perl ${path_herramientas}/annovar/table_annovar.pl ${vcf_in} ${annovar_dbpath} -vcfinput  -buildver hg19 -remove -out ${out_prefix} -protocol dbnsfp35a,gnomad_exome,gnomad_genome,intervar_20180118 -operation f,f,f,f 
...
command {
set -o pipefail
java -Xmx${java_heap_memory_initial} -jar ${path_herramientas}/SnpSift.jar ${parametros} \
${input_vcf} > ${sample_name}.${nombre_step}.vcf
