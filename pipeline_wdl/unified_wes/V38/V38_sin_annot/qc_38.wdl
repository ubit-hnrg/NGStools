...
task cobertura {
    ...
    command {   
        ...
        ${toolpath}bedtools2/bin/sortBed -i ${intervalo_captura} -g ${chromosome_length} > intervalo_sorted.bed
        ...
        samtools view -F1024 -u ${input_bam} -b > bam_nodups.bam
        samtools stats bam_nodups.bam -t intervalo_sorted.bed > ${sample_name}_samtools_nodup_${pipeline_version}.stats ##samtools stat task

        ####global_hist for no dups_bams 
       ${toolpath}bedtools2/bin/coverageBed -a intervalo_sorted.bed -b bam_nodups.bam -hist ${sorted} -g ${chromosome_length} > ${sample_name}_nodup.hist.aux
        ...
        rm global_nodup.header.txt global_nodup.hist ${sample_name}_nodup.hist.aux 
         ...
         samtools stats ${input_bam} -t ${intervalo_captura} > ${sample_name}_samtools_${pipeline_version}.stats
        ...
        ${toolpath}bedtools2/bin/coverageBed -a ${ensembl2intervalo_captura} -b ${input_bam} -hist ${sorted} -g ${chromosome_length} > ${sample_name}.ENS_${pipeline_version}.hist.aux1
        ...
        ${toolpath}bedtools2/bin/genomeCoverageBed -ibam ${input_bam} -bga | awk '$4==0'| ${toolpath}bedtools-2.31.1/bedtools2/bin/intersectBed -a intervalo_sorted.bed -b - > ${sample_name}.no_cubierto_intervalo_${pipeline_version}.tsv
...
# make global tsv report
    ...
    command <<<
        ...
        ## solucion en awk mega rapida enero 2024 agu
        awk -f ${ngs_toolpath}/pipeline_wdl/qualityControl/local_coverage_report_ENS_intersect_Library_hg38.awk ${by_exon_cov} | sort -k1,1 -k4,4 | awk 'BEGIN {print "gene\ttranscriptID\texonNumber\tstart\tend\tstrand\tIntervalLength\tdp>=1\tdp>=10\tdp>=20\tdp>=30\tdp>=50\tdp>=100"} {print}' > ${sample_name}_ENS_local_report.tsv
