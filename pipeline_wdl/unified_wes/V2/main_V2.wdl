...
import './fastq2ubam_V2.wdl' as fastq2ubam 
import './bam2gvcf_V2.wdl' as bamtogvcf
import './ubam2bwa_V2.wdl' as ubam2bwa
#import './jointgenotype_sinrecalibracion_V2.wdl' as joint_genotype
import './jointgenotype_single.wdl' as single_genotypeGVCF

import './quality_control_V2.wdl' as qual_control
import './processMultisampleVCF_V2.wdl' as splitVCF
#import './anotaciones_hnrg_V2.wdl' as anotaciones
 ...
 scatter (sample_txt in array_of_samples_txt)  {
   ...
   call single_genotypeGVCF.singleGenotypeGVCFs {
    ...
    call splitVCF.processJointVCF {
     input:
     singlesampleVCF_restricted = singleGenotypeGVCFs.restricted_vcf,
     single_vcf=singleGenotypeGVCFs.outputvcf,
     #array_path_save = mkdir_samplename.path_out_softlink,
     sample_name=sample_name,

     toolpath = toolpath, 
     region_padded_bed = coord_generator.padded_coord,##Tso_bed
     path_softlink = mkdir_samplename.path_out_softlink,#path_softlink,
...
Array[Pair[String,File]] vcf_x_path = zip (array_path_save_byexon, vcf_individuales)
 ...
 call anotacionesSingle.FuncionalAnnotationSingle {
        input:
        input_vcf = vcf.right,#singleGenotypeGVCFs.restricted_vcf,
        path_save = vcf.left,#mkdir_samplename.path_out_softlink,
        toolpath = toolpath,
        samplename1 = basename(vcf.right,"hg19_singleanno.vcf"),#sample_name,
        java_heap_memory_initial = "12g",
        reference_version = reference_version
  ...
  ##### output jointgenotype
   ...
   File? outputvcf = singleGenotypeGVCFs.outputvcf
   File? outputvcfindex =  singleGenotypeGVCFs.outputvcfindex
   File? detail_metrics_file =  singleGenotypeGVCFs.metrica1
   File? summary_metrics_file = singleGenotypeGVCFs.metrica2
   File? intervalo = singleGenotypeGVCFs.inter
