...
workflow processJointVCF {
 ...
 #    Array[String] path_save
    ...
    #         }
        ...
        call annovar {
            input:
            one_sample_vcf =  singlesampleVCF,#get_individual_vcf.one_sample_vcf,
        ...
        call get_tsv_from_annovar {
            ...
            multisampleVCF = single_vcf,#rename_samples.multisample_vcf_restricted_renamed,
            sample = sample_name,#idsample.idsample,
            #sample1 = sample,
            joinPY = joinPY
        }
...
task annovar{
    ...
    output {
        File annovar_vcf = '${sample}.hg19_singleanno.vcf'
        File annovar_txt = '${sample}.hg19_singleanno.txt'
    }
...
task get_tsv_from_annovar {
    ...
    head -n1 ${annovar_txt} > ${sample}.hg19_singleanno.tsv
    # vuelo las tres columnas de otherinfo
    tail -n+2 ${annovar_txt}|cut -f$nl0,$nl1,$nl2 --complement >>  ${sample}.hg19_singleanno.tsv;
    vcf_header=$(grep '#CH' ${annovar_vcf});

    #remplazo el header
    sed -i "s/Otherinfo/$vcf_header/g" ${sample}.hg19_singleanno.tsv;

    #join one multianno tsv file AND joint genotyped vcf. This script (join_vcf.py) also postprocess Intervar columns.
    python ${joinPY} --multianno_tsv=${sample}.hg19_singleanno.tsv --vcf_multisample=${multisampleVCF} --output=${sample}.multianno_multisample.tsv
    #change dots by tabs.
