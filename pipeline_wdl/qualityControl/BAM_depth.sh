...
awk -F"\t" '{print $1"\t"$7"\t"$8"\t"$9"\t"$4"\t"$5"\t"$2"\t"$3}' $sampleID'_loj.txt' > $sampleID'_loj_sorted_cols.tsv'
$toolpath/bedtools2/bin/intersectBed -a $sampleID'_loj_sorted_cols.tsv' -b $EXON_coords  > $sampleID'_loj_exon_filtered.coverage'

echo -e 'chr\tstart\tend\tcount\tgeneSymbol\texon_number\texon_start\texon_end' > header.tsv
