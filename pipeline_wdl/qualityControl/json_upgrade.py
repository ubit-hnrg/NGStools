...
parser.add_argument('-tf','--trim_front',type=int, help='trimming front bases of fastq for fastp_filtering')
parser.add_argument('-tt','--trim_tail',type=int, help='trimming tail bases of fastq for fastp_filtering')
...
path_out = '/home/hnrg/executionHNRG/upgrade_stats/inputs/'
...
def upgrade_json(samples,bams,bams_index, trim_front,trim_tail,experiment_lib):
    ...
    '''
    experiment_name = ntpath.basename(samples)
    output_name = 'inputs_upgrage_stats_'+experiment_name
    data={}
    ...
    upgrade_json = json.dumps(data, indent = 3)
    return upgrade_json , output_name
    

json_out, out_name = upgrade_json(samples,bams,bams_index, trim_front,trim_tail,experiment_lib)
