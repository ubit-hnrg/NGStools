...
declare -A params=(["XMX"]="12g" ["NT"]="11" ["NCT"]="8" ["QFILTER_READS"]="10" ["MINPRUN"]="3" ["CALL"]="30.0" ["EMIT"]="10.0");
...
function bcl2fastq {
      ...
      echo -n "Please enter bcl dir (absolute path from home):"
      read INPUT

      INPUT=$(trim ${INPUT})
      if [ -z "$INPUT" ]
          then
              echo "Error:bcl directory is null or not exists"
              exit
      fi

      echo -n "Please enter output dir (absolute path from home):"
      read OUTPUT
    ...
    then
      ...
      bcl2fastq --runfolder-dir ${INPUT} -p 8 --output-dir ${OUTPUT} --no-lane-splitting
      if [ $? eq 1 ]
      then
           echo "bcl2fastq Unsuccessful!"
           exit
      fi
  fi
}
