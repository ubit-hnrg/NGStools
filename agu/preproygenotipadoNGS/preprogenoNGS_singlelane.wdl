...
task ScatterIntervalList {
  ...
  command <<<
    set -e
    mkdir out
    java -Dsamjdk.compression_level=${compression_level} -Xmx1g -jar ${path_herramientas}/gatk-package-4.0.8.1-local.jar \
      IntervalListTools \
      -I=${interval_list} \
      -O=out \
...
task HaplotypeCaller {
  ...
  command <<<
      ...
      java -Xmx${haplotypecaller_java_heap_memory_initial} -jar ${path_herramientas}/gatk-package-4.0.8.1-local.jar \
      HaplotypeCaller \
      -R ${ref_fasta} \
      -I ${input_bam} \
...
task MergeVCFs {
  ...
  command {
    java -Xms2000m -jar ${path_herramientas}/gatk-package-4.0.8.1-local.jar \
      MergeVcfs \
      -I=${sep=' -I=' input_vcfs} \
      -O=${output_vcf_name}
  }
...
task ValidateGVCF {
  ...
  command {
    java -Xms3000m -jar ${path_herramientas}/gatk-package-4.0.8.1-local.jar \
      ValidateVariants \
      -V ${input_vcf} \
      --reference ${ref_fasta} \
...
workflow pipeV0 {
  ...
  File wgs_evaluation_interval_list
    ...
    call validarbam {
        ...
        ## Aca se usa scatter si hay una lista de archivos bam
...
# Outputs that will be retained when execution is complete  
  output {
   ...
   File? vcfGenotipado = GenotypeGVCFs.output_vcf
   File? vcfGenotipado_index = GenotypeGVCFs.output_vcf_index
