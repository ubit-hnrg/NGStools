...
while read line; do head -n1 $out/resultado3_all.txt > $(basename $line .bam).txt && grep $(basename $line .bam) $out/resultado3_all.txt >> $out/$(basename $line .bam).txt; done < $bams