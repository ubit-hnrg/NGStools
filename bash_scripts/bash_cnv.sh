...
while read line; do name=$(basename $line .bam); head -n1 $out/resultado3_all.txt > $(basename $line .bam).txt && grep $(basename $line .bam) $out/resultado3_all.txt >> $out/$name.txt; done < $bams